...
function importFiling(job, callback) {
    task = job;
    if ('data' in job) {
        task = job.data;
    }
    ...
    function error(err) {
        ...
        if (transaction !== null) {
            ...
            transaction.rollback().then(callback.bind(this, err)).catch(() => {
    ...
    function done() {
        ...
        console.info(
            chalk.green(
                `inserted ${numeral(processed).format()} rows from ${filing_id}`
            )
        );
    ...
    function startTransaction(cb) {
        models.sequelize.transaction().then(cb).catch(error);
    }
    ...
    function notify(channel, data) {
        if (models.sequelize.getDialect() == 'postgres') {
            ...
            return models.sequelize.query(
                `NOTIFY ${channel},  ${models.sequelize.escape(
                    JSON.stringify(data)
                )};`
            );
        }
                            ...
                            if (processed !== 0 && processed % payload === 0) {
                                ...
                                console.info(
                                    chalk.gray(
                                        `processed ${numeral(
                                            processed
                                        ).format()} records in ${numeral(
                                            elapsed
                                        ).format('0.00')} seconds at ${numeral(
                                            processed / elapsed
                                        ).format()} rows/second`
                                    )
                                );
    ...
    checkForFiling(filing_id, () => {
        startTransaction(t => {
            ...
            processFiling(function (cb) {
                cb(null,request.get(task.location));
            }, done); // openStream
        });
    });
}

module.exports = new FilingQueue(importFiling);
